...
1. **Tactics editor (v2) — keep drawing** — drag-to-place holds, kill points, routes, routed kill-points (with a waypoint budget), authored facing/crossfires, death/contact/time triggers, **utility lineups**, and **walkability feedback** are in; next are N-step chains, A/B mirror + a play library, and **attack-side plays** (executes, lurks, defaults as authored geometry + lineups, built on the route primitive) — the big one, since the attack is half the game.
