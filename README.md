...
1. **The tactics editor** — author per-player routes, holds, and timing on the minimap (the same navmesh the engine resolves on), turning the dials + roles above into a drawn play. This is where "how owners build tactics" actually lives.
2. **The persistent world** — scheduling, the resolution worker, accounts, clubs (NestJS + Supabase + Stripe, Phase 2) — where these match inputs (roster, comp, tactics) come from real owners instead of the sample.
3. **More maps** — once asset egress is allowlisted (or PNGs are committed), every map lights up through the same alpha→navmesh pipeline; staying 2D, with an optional per-map height layer only if a vertical map needs it.
