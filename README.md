...
1. **Tactics editor (v2) — draw the play** — the engine already runs authored per-player positions + kill points; next is the *canvas*: drag each player's hold, route, and rotation on the minimap (the same navmesh the engine resolves on) and re-sim live, plus more triggers (on-contact, at-time) and attack-side plays.
