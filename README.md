...
1. **Tactics editor (v2) — keep drawing** — drag-to-place holds + kill points are in; next are **routes** (not just a hold point but the path to it), more triggers (on-contact, at-time, N-step chains), and **attack-side plays** (executes, lurks, defaults as authored geometry) — the big one, since the attack is half the game.
