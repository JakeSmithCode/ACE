...
1. **Tactics editor (v2) — keep drawing** — drag-to-place holds, kill points, and **routes** are in; next are routed kill-points (give the rotation its own path), more triggers (on-contact, at-time, N-step chains), and **attack-side plays** (executes, lurks, defaults as authored geometry, built on the route primitive) — the big one, since the attack is half the game.
