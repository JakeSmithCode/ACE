...
1. **Comp as an input too** — agent picks (not just roles) drawn from the patch's agent tiers and each player's mastery, so the *composition* you field matters alongside the tactics.
2. **The persistent world** — scheduling, the resolution worker, accounts, clubs (NestJS + Supabase + Stripe, Phase 2) — where these match inputs come from real owners.
