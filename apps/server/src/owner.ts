...
const load = (store: WorldStore, id: string) => {
...
export function claim(store: WorldStore, id: string, clubId: string, account: string): WorldClub {
  const w = claimClub(load(store, id), clubId, account);
  store.saveWorld(id, w);
...
export function revert(store: WorldStore, id: string, clubId: string): void {
  store.saveWorld(id, revertClub(load(store, id), clubId));
}
...
export function savePlan(store: WorldStore, id: string, clubId: string, plan: ClubPlan): void {
  store.saveWorld(id, setClubPlan(load(store, id), clubId, plan));
}

/** The club an account owns in this world (null if none) — what `GET /me` returns. */
export function myClub(store: WorldStore, id: string, account: string): WorldClub | undefined {
  return clubOf(load(store, id), account);
}

/** A club's current plan — what the tick reads and `GET /me/plan` returns. */
export function clubPlan(store: WorldStore, id: string, clubId: string): ClubPlan {
