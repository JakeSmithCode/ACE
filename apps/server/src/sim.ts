...
function clubPlan(self: WorldClub, opp: WorldClub, patch: WorldState['patch'], fitness?: Fitness, morale?: Morale): { team: Team; comp: Comp; tactics: WorldClub['tactics'] } {
...
export function fullSimResolver(w: WorldState, navOf: (map: MatchInput['map']) => Navmesh, forks = 0): SimResolver {
  ...
  const resolve = (fx: Fixture, seed: number): MatchResult => {
    ...
    const h = clubPlan(home, away, w.patch, w.fitness, w.morale), a = clubPlan(away, home, w.patch, w.fitness, w.morale);
