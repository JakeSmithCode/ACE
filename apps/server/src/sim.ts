...
import { buildMatchInput, clubTeam, fixtureMap, aiMatchupTactics, aiComp, aiBestFive, planFive, fitTeam, type WorldState, type WorldClub, type Fixture, type MatchResult, type Fitness } from '@ace/world';
...
function clubPlan(self: WorldClub, opp: WorldClub, patch: WorldState['patch'], fitness?: Fitness): { team: Team; comp: Comp; tactics: WorldClub['tactics'] } {
  // a HUMAN owner's authored plan, with match-night FITNESS applied: the fielded five honours
  // injuries (a reserve covers, or he plays hurt) and fatigue dulls his attributes — so
  // rotating depth in is a real lever. No fitness state → fresh team (byte-identical).
  if (self.owner) return { team: fitTeam(self, planFive(self), fitness).team, comp: self.comp, tactics: self.tactics };
...
export function fullSimResolver(w: WorldState, navOf: (map: MatchInput['map']) => Navmesh, forks = 0): SimResolver {
  ...
  const resolve = (fx: Fixture, seed: number): MatchResult => {
    ...
    const h = clubPlan(home, away, w.patch, w.fitness), a = clubPlan(away, home, w.patch, w.fitness);
