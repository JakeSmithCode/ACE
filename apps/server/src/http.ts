...
export async function startLiveServer(opts: LiveServerOpts = {}): Promise<LiveServer> {
  ...
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (path[0] === 'me' && path.length === 1) {
      ...
      // the FINANCE projection — a live "if the season ended today" P&L (the ledger lines
      // behind the settle-net number, at the current division rank)
      const myWins = meRows.filter(f => fixtureStatus(f, clock()) === 'resolved' && ((f.home === ci && f.homeScore > f.awayScore) || (f.away === ci && f.awayScore > f.homeScore))).length;
      const objOnTrack = !!c.boardObjective && objectiveRank > 0 && objectiveRank <= c.boardObjective.needRank;
      const sponsorOnTrack = !!c.sponsor && sponsorGoalMet(c.sponsor.goal, c.sponsor.goalN, { objMet: objOnTrack, finish: objectiveRank, divSize: meTable.length, promo: wm.promo, wins: myWins });
      const finance = projectFinance({
        rank: objectiveRank, divSize: meTable.length, tier: c.tier, balance: c.balance,
        ticketIncome: c.fans ? Math.round(c.fans * FAN_TICKET) : 0,
        sponsorBase: c.sponsor?.base ?? 0, sponsorBonus: c.sponsor?.bonus ?? 0, sponsorOnTrack,
