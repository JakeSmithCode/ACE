...
export interface SquadPlayer { id: string; handle: string; role: string; age: number; overall: number; value: number; starter: boolean; igl: boolean; ceiling: [number, number]; room: number; attrs: AttrScout[]; agents: { agent: string; level: number }[] }
export function squadView(w: WorldState, c: WorldClub): SquadPlayer[] {
  ...
  return c.roster.map(p => {
    ...
    return { id: p.id, handle: p.handle, role: p.role, age: p.age, overall: ovr, value: playerValue(p, w.patch), starter: five.has(p.id), igl: !!p.igl, ceiling, room: Math.max(0, ceiling[1] - ovr), attrs: scoutedAttrs(p, true, 0), agents };
