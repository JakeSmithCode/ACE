...
let world: WorldState = createWorld(seed, { tiers: 11, size: 16, promo: 2, region: 'AMER' });
...
const w2 = (() => { let w = createWorld(seed, { tiers: 11, size: 16, promo: 2, region: 'AMER' }); for (let s = 0; s < seasons; s++) { w = simulateSeason(w); w = advanceWorld(w).world; } return w; })();
