...
Trading is in (round dynamics 1 of 3). `duel` now takes a numeric `surprise` edge (not `-1|0|1`); resolveRound composes it from vision (`±FIRST_SHOT`), pulses (`±FIRST_SHOT`), then a **trade**: a kill sets `winnerAg.exposedUntil = t + TRADE_WINDOW`, and a teammate who can see that exposed killer gets `±TRADE_EDGE` (via `Math.max/min`, so it flips a held first-shot but never weakens a bigger same-side edge). `exposedUntil` must reset to `-1` in the fork clones (it does) or odds drift. Trading favours the grouped side, so it shifted balance ~+13pt to attackers — countered by raising `HOLD_BONUS` 6→9 (a held angle is worth more when it's punishable). ~37% trade rate; ~52% attacker. The real defensive rebalancers are the next two round-dynamics steps (info-gated rotations, retakes). Viewer tags trades in the kill feed (⇄) — derived from the event stream, no contract change. Tuning knobs added: `TRADE_WINDOW`, `TRADE_EDGE`, and `HOLD_BONUS` is now a primary balance lever (raises stalls if too high).
